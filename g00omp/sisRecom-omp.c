...
j = A[n][1];
    for (k = 0; k < nCaracteristicas; k++) {
        privLsum[meuId][i][k] += alpha * ( 2 * ( A[n][2] - B[i][j] ) * ( -RT[j][k] ) );
        privRTsum[meuId][j][k] += alpha * ( 2 * ( A[n][2] - B[i][j] ) * ( -L[i][k] ) );
    }
}
...
void loop() {
    #pragma omp parallel if (nCaracteristicas > 50 || nNaoZero > 50)
    {
        ...
        #pragma omp for
        for (int nt = 0; nt < numThreads; nt++) {
           privLsum[nt]  = (double **)malloc(nUsuarios * sizeof(double *));
         privRTsum[nt] = (double **)malloc(nItens * sizeof(double *));    }

       for (int nt = 0; nt < numThreads; nt++) {
    for (int i = 0; i < nUsuarios; i++) {
        privLsum[nt][i] = (double *)malloc(nCaracteristicas * sizeof(double));
        for (int j = 0; j < nCaracteristicas; j++) {
            privLsum[nt][i][j] = 0.0;
        }
    }
    for (int i = 0; i < nItens; i++) {
        privRTsum[nt][i] = (double *)malloc(nCaracteristicas * sizeof(double));
        for (int j = 0; j < nCaracteristicas; j++) {
            privRTsum[nt][i][j] = 0.0;
        }
    }
}
